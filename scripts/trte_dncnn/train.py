...
def main():
    ...
    # -- get/run experiments --
    exps,uuids = cache_io.train_stages.run("exps/trte_dncnn/train.cfg",
                                           ".cache_io_exps/trte_dncnn/train/",
                                           update=True)
    print(exps[0])
    print(len(exps))
    def clear_fxn(num,cfg): return True
    results = cache_io.run_exps(exps,train.run,uuids=uuids,preset_uuids=True,
                                name=".cache_io/trte_dncnn/train",
                                version="v1",skip_loop=False,clear_fxn=clear_fxn,
                                clear=False,enable_dispatch="slurm",
                                records_fn=".cache_io_pkl/trte_dncnn/train.pkl",
